...
rcswalk_delta_orig(RcsFile *rcs, RcsWalker *walker, RcsVersion *version, int *count, void *data) {
    ...
    for (c = version->children; c; c = c->next) {
        ...
        if ((ret = walker->delta_orig(rcs, reverse ? child : version, reverse ? version : child,
                                      data))) {
            return ret;
        }
...
rcswalk_load(RcsFile *rcs, gboolean *skip) {
    ...
    while (fgets(oneline, 1024, rlog)) {
        ...
        } else if (read_state == BAR_STATE &&
                   rcswalk_match(&oneline_p, "----------------------------")) {
...
stat_int_histogram(IntStat *stat) {
    ...
    for (i = 0; i < stat->count; i += consec) {
        ...
        fprintf(p_out, "%qd, %0.3f\n", g_array_index(stat->values, long long, i),
                (double) (i + consec) / (double) stat->count);
        fprintf(s_out, "%qd, %0.3f\n", g_array_index(stat->values, long long, i),
                (double) cum / (double) stat->sum);
    }
...
stat_dbl_histogram(DblStat *stat) {
    ...
    for (i = 0; i < stat->count; i += consec) {
        ...
        fprintf(p_out, "%0.6f, %0.3f\n", g_array_index(stat->values, double, i),
                (double) (i + consec) / (double) stat->count);
...
config_help(void) {
    ...
    fprintf(stderr,
            "Expecting the following options in one or more config files on the command line:\n");
