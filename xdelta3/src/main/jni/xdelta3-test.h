...
test_address_cache(xd3_stream *stream, int unused) {
    ...
    for (offset = 0, outp = ADDR_HEAD(stream);
         outp != NULL; offset += outp->next, outp = outp->next_page) {
...
test_command_line_arguments(xd3_stream *stream, int ignore) {
    ...
    for (i = 0; i < pairs; i += 1) {
        ...
        if (ratio >= TEST_ADD_RATIO + TEST_EPSILON) {
            XPR(NT
            "test encode with size ratio %.4f, "
            "expected < %.4f (%"
            Q
            "u, %"
            Q
    ...
    for (i = 0; i < SIZEOF_ARRAY(match_tests); i += 1) {
        ...
        *rptr = 0;
        while (!xd3_rlist_empty(&stream->iopt_used)) {
            ...
            snprintf_func(rptr, rbuf + TESTBUFSIZE - rptr, "%"
            W
            "u/%"
            W
            "u",
        ...
        if (strcmp(rbuf, test->result) != 0) {
            ...
            "test %"
            W
            "u: expected %s: got %s", i, test->result, rbuf);
...
#define DO_TEST(fn, flags, arg)                                         \
  do {                                                                \
    ...
    IF_LZMA(
    DO_TEST (decompress_single_bit_error, XD3_SEC_LZMA, 54));
    IF_FGK(
    DO_TEST (decompress_single_bit_error, XD3_SEC_FGK, 3));
    IF_DJW(
    DO_TEST (decompress_single_bit_error, XD3_SEC_DJW, 8));
    ...
    IF_LZMA(
    DO_TEST (secondary_lzma_default, 0, 0));
#endif

    IF_LZMA(
    DO_TEST (secondary_lzma, 0, 1));
    IF_DJW(
    DO_TEST (secondary_huff, 0, DJW_MAX_GROUPS));
    IF_FGK(
    DO_TEST (secondary_fgk, 0, 1));

    DO_TEST (compressed_stream_overflow, 0, 0);
    IF_LZMA(
    DO_TEST (compressed_stream_overflow, XD3_SEC_LZMA, 0));
