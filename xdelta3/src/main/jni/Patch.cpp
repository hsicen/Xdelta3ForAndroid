...
jint nativePatch(JNIEnv *env, jobject thiz, jint encode, jstring inPath,
    ...
    inUTFPath = env->GetStringUTFChars(inPath, nullptr);
    InFile = fopen(inUTFPath, "rb");
    if ((int) (size_t) InFile <= 0) {
        res = -1;
        goto out3;
    }

    env->ReleaseStringUTFChars(inPath, inUTFPath);
    srcUTFPath = env->GetStringUTFChars(srcPath, nullptr);
    SrcFile = fopen(srcUTFPath, "rb");
    if ((int) (size_t) SrcFile <= 0) {
        res = -1;
        goto out2;
    }

    env->ReleaseStringUTFChars(srcPath, srcUTFPath);
    outUTFPath = env->GetStringUTFChars(outPath, nullptr);
    OutFile = fopen(outUTFPath, "wb");
    if ((int) (size_t) OutFile <= 0) {
        res = -1;
        goto out1;
    }
