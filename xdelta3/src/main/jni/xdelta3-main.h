...
main_atoux(const char *arg, xoff_t *xo, xoff_t low,
    ...
    if (x < low) {
        XPR(NT "-%c: minimum value: %"
        Q
        "u\n", which, low);
        return EXIT_FAILURE;
    }
    if (high != 0 && x > high) {
        XPR(NT "-%c: maximum value: %"
        Q
        "u\n", which, high);
        return EXIT_FAILURE;
    }
 ...
 case CMD_MERGE_ARG:
   ...
   break;
                ...
                if (IS_ENCODE (cmd) || cmd == CMD_DECODE || cmd == CMD_RECODE) {
                    if (!option_quiet && IS_ENCODE (cmd) &&
                        ...
                        if (option_verbose > 1 &&
                            ...
                            XPR(NT "warning: input position %"
                            Q
                            "u overflowed "
                            "instruction buffer, needed %"
                            W
                            "u (vs. %"
                            W
                            "u), "
    ...
    if (option_verbose > 1 && cmd == CMD_ENCODE) {
        ...
        XPR(NT "target hash table size: %"
        W
        "u\n", stream.small_hash.size);
        if (sfile != NULL && sfile->filename != NULL) {
            XPR(NT "source hash table size: %"
            W
    ...
    if (option_verbose > 2 && cmd == CMD_ENCODE) {
        XPR(NT "source copies: %"
        Q
        "u (%"
        Q
        "u bytes)\n",
                stream.n_scpy, stream.l_scpy);
        XPR(NT "target copies: %"
        Q
        "u (%"
        Q
        "u bytes)\n",
    ...
    while (my_optstr) {
        ...
        switch (ret) {
            ...
            case 'S':
                ...
                break;
            case 'A':
                if (my_optarg == NULL) { option_use_appheader = 0; }
                else { option_appheader = (uint8_t *) my_optarg; }
                break;
            case 'B': {
                xoff_t bsize;
